...
<head>
    ...
    <style>
        ...
        body, html {
            ...
            cursor: url('./mous.cur'), auto; /* 设置自定义鼠标指针 */
        }
        .bg-video {
            position: fixed;
            top: 0;
        ...
        .play-button {
            ...
            font-size: 2em; /* 字体大小 */
            color: #fff; /* 文字颜色 */
            cursor: url('./mous.cur'), pointer; /* 设置自定义鼠标指针 */
        }
        /* 烟花效果 */
        canvas {
            position: fixed;
...
<body>
    <video class="bg-video" id="bg-video" autoplay muted>
        ...
        你使用的浏览器暂不支持此网站<a href="./MicrosoftEdgeSetup.exe" style="color: #EE82EE;">点击此处</a>下载支持的浏览器。
        ——GWLMC
    </video>
    ...
    <div class="content" id="content">
        ...
        <h3 style="color: #00BFFF;" id="server-ip">
            MC(2025人)服务器状态：加载中...
        </h3>
    ...
    <script>
        ...
        fetch('https://gwlmcyp.cpolar.top/McIP/IP.txt')
            .then(response => response.text())
            .then(ip => {
                serverIpElement.textContent = `MC(2025人)服务器IP：${ip}`;
            })
            .catch(error => {
                console.error('Error:Data/Ping_Error', error);
                serverIpElement.textContent = 'GWLMC服务器暂未启动，请联系管理员。';
            });
    </script>
</body>
</html>
